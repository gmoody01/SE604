...
for i in range(0, num_of_var):
    ...
    if num_of_steps == 'No':
        ...
        var_ranges[i] = list(chain(range(var_start_val[i], var_end_val[i] + var_step_val[i], var_step_val[i]),
...
# inputs we have. Still have to include the else statement for the third variable.
if num_of_var == 1: # One variable
    ...
    row2_spacing_index = starting_row + 2
    ...
    for i in var_ranges[num_of_var - 3]:
        ...
        row1_spacing_index += num_of_loop_var + 1

#                first_var_value_ID = convertToTitle(col2num(starting_col + 1))
#            ws[starting_col + str(starting_row)] = var_naming[var - 1] + ' (' + var_unit[var - 1] + ')'
#            ws[first_var_value_ID + str(starting_row)] = var_ranges[0, 0]
